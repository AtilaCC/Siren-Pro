...
async def claude_detect_narratives(tokens: list) -> dict:
    ...
    token_list = "\n".join([
        f"${t['sym']} | score:{t['score']} | chg:{t['chg']:+.1f}% | chain:{t['chain']}"
        for t in top20
    ])
...
Identifique narrativas temáticas (ex: AI tokens, memecoins, DeFi, L2, gaming, etc.)
Qual narrativa está dominando? Qual blockchain está em alta?

Responda em JSON:
{{"dominant_narrative":"nome","tokens_in_narrative":["SYM1","SYM2"],"hot_chain":"ethereum","insight":"observação breve","confidence":75}}"""
