...
def api_positions():
    try:
        ...
        c  = db.cursor()
        c.execute("""
            SELECT id, symbol, entry, size, stop, take_profit, status, created_at, exit_price
            FROM trades_v2
        ...
        cols = ["id", "symbol", "entry", "size", "stop", "take_profit", "status", "created_at", "exit_price"]
        positions = [dict(zip(cols, r)) for r in rows]

        total     = len(positions)
        open_pos  = sum(1 for p in positions if p["status"] == "OPEN")
        total_pnl = sum(
            ((p["exit_price"] or 0) - p["entry"]) / p["entry"] * p["size"]
            for p in positions if p["status"] == "CLOSED" and p["exit_price"]
        )
