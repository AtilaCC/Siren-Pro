...
def analyze_token_public():
    """
    POST /api/ai/analyze-token  (sem autenticação)
    Body: {"prompt": "..."}
    Retorna {"text": "..."} com análise Claude do token.
    """
    ...
    if not result:
        return jsonify({"error": "IA indisponível"}), 503

    return jsonify({"text": result})


@routes_bp.route("/api/ai/analyze-market", methods=["POST"])
def analyze_market_public():
    """
    POST /api/ai/analyze-market  (sem autenticação)
    Body: {"prompt": "..."}
    Retorna {"text": "..."} com análise Claude de mercado.
    """
    data   = request.get_json(silent=True) or {}
    prompt = (data.get("prompt") or "").strip()
