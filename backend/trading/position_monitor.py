...
Parâmetros configuráveis via env:
  MONITOR_INTERVAL_SECONDS  — intervalo do loop (padrão: 60s)
...
def _get_current_price(symbol: str) -> float:
    ...
    Em paper mode usa a API pública da Binance (sem autenticação).
    Valida o símbolo no cache antes de chamar a API para evitar erro -1121.
    Retorna 0.0 em caso de falha ou símbolo inválido.
    """
    try:
        ...
        # Valida apenas se o cache já foi carregado
        if SYMBOLS_CACHE and not validate_symbol(pair):
            log.warning(f"_get_current_price ${symbol}: símbolo não listado na Binance — ignorando")
            return 0.0

        return get_price(pair)
    except Exception as e:
        log.warning(f"_get_current_price ${symbol}: {e}")
        return 0.0
