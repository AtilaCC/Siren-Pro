...
def execute_signal(token: dict, label: str, user_id: int = None) -> dict:
    ...
    # Valida o par na Binance em AMBOS os modos (paper e real)
    # Evita tentar executar tokens Alpha sem par USDT negociável
    if not validate_symbol(pair):
        log.warning(f"Par {pair} não encontrado na Binance — ignorando silenciosamente")
        return {"success": False, "error": f"Par {pair} não negociável na Binance Spot"}
