...
def execute_signal(token: dict, label: str, user_id: int = None) -> dict:
    ...
    # ── CHECAR DUPLICATA — não abre se já tem posição aberta ──────────────
    try:
        from trading.position_monitor import has_open_position
        if has_open_position(sym_clean):
            log.info(f"[{mode}] ${sym} já tem posição aberta — ignorando")
            return {"success": False, "error": "Posição já aberta"}
    except Exception:
        pass  # se falhar a checagem, continua normalmente

    # ── VERIFICAR POSIÇÃO DUPLICADA ───────────────────────────────────────
    try:
        from trading.position_monitor import has_open_position
        if has_open_position(sym):
            log.info(f"[{mode}] ${sym} já tem posição aberta — ignorando duplicata")
            return {"success": False, "error": "Posição já aberta"}
    except Exception:
        pass  # Se falhar a checagem, continua (melhor entrar do que travar)
    ...
    if pair is None:
        if ENABLE_REAL:
            ...
            # PAPER: ignora tokens que só existem no Alpha (sem par Spot negociável)
            log.info(f"[PAPER] Par {sym_clean}USDT não existe na Binance Spot — ignorando token Alpha")
            return {"success": False, "error": f"Token Alpha sem par Spot negociável"}
...
def _save_trade(
    ...
    try:
        ...
        tp_price   = round(price * 1.10, 8)

        # Tenta inserir em trades_v2 sem user_id primeiro (schema mais comum)
        try:
            c.execute(
                ...
                (sym, side, qty, price, total_usdt, stop_price, tp_price,
                 score, result, order_id, result, int(time.time())),
            )
        except Exception:
            # Fallback: tenta com user_id
            db.rollback()
            c.execute(
                """INSERT INTO trades_v2
        ...
        # Tabela antiga para compatibilidade
        try:
            c.execute(
                """INSERT INTO trades
        ...
        db.commit()
        db.close()
        log.info(f"Trade salvo: {side} ${sym} qty={qty} SL={stop_price} TP={tp_price}")
    except Exception as e:
