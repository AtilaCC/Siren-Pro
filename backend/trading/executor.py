...
Fluxo OBRIGATÓRIO (nenhuma etapa pode ser pulada):
  ...
  4. Verificar saldo
  5. Calcular posição via risk_manager (SL DINÂMICO baseado em volatilidade)
...
# Volatilidade padrão se não houver dados (conservador)
DEFAULT_VOLATILITY_PCT = float(os.environ.get("DEFAULT_VOLATILITY_PCT", "5.0"))


# ══════════════════════════════════════════════
# SL/TP DINÂMICO BASEADO EM VOLATILIDADE
# ══════════════════════════════════════════════

def _calc_dynamic_sl_tp(token: dict, price: float) -> tuple[float, float, float]:
    """
    Calcula Stop Loss e Take Profit dinamicamente com base na
    volatilidade real do token (chg 24h como proxy de volatilidade).

    Lógica:
    - SL = 1.2x a volatilidade diária (para não ser stopado por ruído normal)
    - TP = 2.5x o SL (Risk/Reward mínimo 1:2.5)
    - Limites: SL entre 3% e 12%, TP entre 7% e 30%

    Retorna: (stop_price, tp_price, sl_pct)
    """
    # Volatilidade baseada na variação 24h (valor absoluto)
    chg_abs = abs(token.get("chg", DEFAULT_VOLATILITY_PCT))

    # Normaliza: tokens muito voláteis têm SL maior, menos voláteis menor
    if chg_abs > 30:
        sl_pct = 10.0   # token muito volátil — SL largo
    elif chg_abs > 15:
        sl_pct = 7.0
    elif chg_abs > 5:
        sl_pct = 5.0
    else:
        sl_pct = 3.5    # token estável — SL apertado

    # Ajuste: se RSI alto (sobrecomprado), SL mais conservador
    rsi = token.get("rsi", 50)
    if rsi > 70:
        sl_pct += 1.5

    # Ajuste: se volume fraco, SL mais conservador (spreads maiores)
    vol = token.get("vol", 0)
    if vol < 200_000:
        sl_pct += 1.0

    # Garante limites
    sl_pct = max(3.0, min(12.0, sl_pct))

    # TP = 2.5x o risco (R:R mínimo de 1:2.5)
    tp_pct = sl_pct * 2.5
    tp_pct = max(7.0, min(30.0, tp_pct))

    stop_price = round(price * (1 - sl_pct / 100), 8)
    tp_price   = round(price * (1 + tp_pct / 100), 8)

    return stop_price, tp_price, sl_pct
...
async def execute_signal_async(token: dict, label: str, user_id: int = None) -> dict:
    ...
    # ── 5. Verificar saldo e calcular posição ─────────────────────────────
    ...
    # ── 6. SL/TP DINÂMICO ────────────────────────────────────────────────
    stop_price, tp_price, sl_pct = _calc_dynamic_sl_tp(token, price)
    log.info(f"[{mode}] ${sym} | SL={sl_pct:.1f}% (${stop_price:.8f}) | TP=${tp_price:.8f}")
