...
# ═══════════════════════════════════════
# MELHORIA 2: SENTIMENT TELEGRAM (grupos públicos)
# ═══════════════════════════════════════

_tg_mentions: dict = {}  # {sym: count_nas_ultimas_1h}

async def fetch_telegram_sentiment(session, tokens: list) -> dict:
    """
    Monitora menções de tokens no Telegram público via t.me/s/ (sem API key).
    Grupos públicos de crypto BR: BinanceAlpha, CryptoAlertsBR, etc.
    Gratuito, sem autenticação.
    """
    GRUPOS = [
        "https://t.me/s/binancealpha",
        "https://t.me/s/CriptoAlertasBR",
        "https://t.me/s/cryptoalphabr",
    ]
    syms = {t["sym"].upper() for t in tokens[:50]}  # top 50 por score
    mentions = {s: 0 for s in syms}
        ...
        try:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=8),
                ...
                if r.status == 200:
                    ...
                    for sym in syms:
                        count = text_upper.count(f"${sym}") + text_upper.count(f" {sym} ")
                        mentions[sym] = mentions.get(sym, 0) + count
        except Exception:
            continue

    hot = {k: v for k, v in mentions.items() if v > 0}
    if hot:
        log.info(f"Telegram sentiment: {len(hot)} tokens mencionados | top: {sorted(hot.items(), key=lambda x: -x[1])[:3]}")
    return mentions



# ═══════════════════════════════════════
# MELHORIA 3: GOOGLE TRENDS (gratuito)
# ═══════════════════════════════════════

_trends_cache: dict = {}  # {sym: interest_score 0-100}
_trends_ts: float = 0

async def fetch_google_trends(tokens: list) -> dict:
    """
    Busca interesse de busca no Google Trends para top tokens.
    Gratuito, sem API key. Atualiza a cada 2h.
    """
    import time
    global _trends_cache, _trends_ts
    if time.time() - _trends_ts < 7200:  # cache 2h
        return _trends_cache

    try:
        from pytrends.request import TrendReq
        top_syms = [t["sym"] for t in sorted(tokens, key=lambda x: -x["score"])[:5]]
        pytrends = TrendReq(hl="pt-BR", tz=360, timeout=(10, 25))
        pytrends.build_payload(top_syms, timeframe="now 1-d", geo="BR")
        df = pytrends.interest_over_time()
        if not df.empty:
            result = {}
            for sym in top_syms:
                if sym in df.columns:
                    result[sym] = int(df[sym].iloc[-1])
            _trends_cache = result
            _trends_ts = time.time()
            trending = {k: v for k, v in result.items() if v > 30}
            if trending:
                log.info(f"Google Trends: {trending}")
            return result
    except Exception as e:
        log.warning(f"Google Trends falha: {e}")
    return {}
