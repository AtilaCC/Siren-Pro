...
def _can_open_trade(sym: str, t: dict) -> tuple[bool, str]:
    """
    Portão completo antes de qualquer execute_signal().
...
def _register_trade_open(sym: str) -> None:
    """Registra abertura de posição. Chamado APÓS execute_signal bem-sucedido."""
    with _trade_state_lock:
...
async def send_alerts(session, tokens: list, narrative: dict = None):
    ...
    for t in tokens:
        ...
        for key, label, msg, label_short in alerts_to_check:
            ...
            # ── Auto-trade: portão completo antes de executar ──
            if key in AUTO_TRADE_LABELS and t["score"] >= AUTO_TRADE_MIN_SCORE:
                ...
                if not approved:
                    ...
                    try:
                        result = execute_signal(t, label_short)
                        if result["success"]:
                            _register_trade_open(sym)
                            log.info(
                                f"[AUTO-TRADE] ✅ ${sym} {label_short} | "
                                f"mode={result['mode']} size=${result['total_usdt']:.2f} "
                                f"SL={result['stop_price']} TP={result['tp_price']} "
                                f"RR={result.get('rr', '?')}"
                            )
                        else:
                            log.info(f"[AUTO-TRADE] ⏭ ${sym} ignorado: {result.get('error','?')}")
...
# ═══════════════════════════════════════
# BACKTEST AVANÇADO (movido para engine)
# ═══════════════════════════════════════
