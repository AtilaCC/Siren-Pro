...
async def send_alerts(session, tokens: list, narrative: dict = None):
    ...
    for t in tokens:
        ...
        # PUMP: limiar elevado para 15% + RSI < 65 (evita entrar no topo)
        if t["chg"] >= 15 and t["rsi"] < 65:
            body = [
        ...
        for key, label, msg, label_short in alerts_to_check:
            ...
            # PRÉ-PUMP e REVERSÃO: threshold menor (70) — entram antes do movimento
            _auto_min = 70 if key in ("pre", "rev", "rsi") else AUTO_TRADE_MIN_SCORE
            if key in AUTO_TRADE_LABELS and t["score"] >= _auto_min:
                t["entry_signal"] = key  # propaga para passes_entry_quality
