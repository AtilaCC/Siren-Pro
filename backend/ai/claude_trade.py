...
Se TODOS esses critérios forem verdadeiros simultaneamente:
  → chg_24h > 15%  ← threshold reduzido (antes era 25%)
  → vol_growth_pct > 15% (volume crescendo de verdade)
...
Exemplos:
  chg=29% + vol_growth=30% + RSI=55 + score=78 → TREND, conf=0.78
  chg=42% + vol_growth=50% + RSI=54 + score=80 → TREND, conf=0.85
...
def _pre_filter(market_data: dict) -> tuple[bool, str]:
    ...
    # Stablecoins e tokens pegged — nunca vão atingir TP
    sym = market_data.get("symbol", "").upper()
    STABLE_TOKENS = {"USD1", "USDC", "FDUSD", "DAI", "TUSD", "BUSD", "USDD", "USDP", "FRAX", "LUSD"}
    if sym in STABLE_TOKENS or (sym.startswith("USD") and len(sym) <= 5):
        return False, f"{sym} é stablecoin — bloqueado"
