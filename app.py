...
app.config["JWT_ACCESS_TOKEN_EXPIRES"] = False   # sem expiração (ajuste conforme necessidade)
jwt = JWTManager(app)
...
def _run_monitor_thread():
    """Executa o monitor de posições em thread separada."""
    try:
...
def _run_bot_thread():
    """Executa o loop assíncrono do bot em thread separada."""
    try:
        asyncio.run(run_bot())
    except Exception as e:
...
# Executa startup ao importar (Railway / Gunicorn fazem import do módulo)
startup()
